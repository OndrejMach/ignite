  ...
  def apply(entry: OrderDBInput  ): OrderDBStage = {
    ...
    def transformString(i: Int, arrayElementNotExist: String, f: (String) => String): Option[String] = {
      if (hotspot_vec.size > i && !hotspot_vec(i).isEmpty) {
        if (hotspot_vec(i) == "*") None else Some(f(hotspot_vec(i)))
    ...
    val hotspot_venue_type_code = transformString(2, "UNDEFINED", (s: String) => s.trim())
    val hotspot_venue_code = transformString(3, "UNDEFINED", (s: String) => s.trim())
    val hotspot_timezone = if (hotspot_vec.size > 6 && !hotspot_vec(6).isEmpty) {
      Some(hotspot_vec(hotspot_vec.size-1))
    }
    else Some("UNDEFINED")
    val hotspot_ident_code = {
      if (hotspot_vec.size > 5 && !hotspot_vec(hotspot_vec.size-2).isEmpty) {
        if (hotspot_vec(hotspot_vec.size-2) == "*" || hotspot_vec(hotspot_vec.size-2).startsWith("DEFAULT") || hotspot_vec(hotspot_vec.size-2).startsWith("000000000000")) {
        ...
        } else Some(hotspot_vec(hotspot_vec.size-2))
      } else Some("UNDEFINED")
    }
    val hotspot_city_code: Option[String] = {
      if (hotspot_vec.size > 4 && !hotspot_vec(4).isEmpty) {
        if (hotspot_vec(4) == "*") {
          ...
          Some((for {i <- 4 until (hotspot_vec.size-2)} yield hotspot_vec(i)).reduce(_ + "/" + _))
        }
      } else {
        Some("UNDEFINED")
      }
    ...
    val requestDate = Date.valueOf(LocalDate.parse(entry.transaction_date.get, DateTimeFormatter.ofPattern("yyyyMMdd")))
    ...
    new OrderDBStage(
      ...
      ta_request_date = Some(requestDate),
      ...
      email = if (entry.email.isDefined && !entry.email.get.isEmpty) Some(entry.email.get.trim) else Some("#"),
      amount = entry.amount,
