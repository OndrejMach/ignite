...
class FailedLoginProcessor(failedLoginReader: Reader, hotspotData: DataFrame, citiesData: DataFrame)(implicit sparkSession: SparkSession) extends Logger {
  ...
  private lazy val cityDataforLookup =
    citiesData.select("city_code", "city_id")
