...
dag = DAG(
    dag_id='SIT_inflight_processing_daily',
    default_args=default_args,
    description='SIT_inflight_processing_daily',
...
def getSparkSubmit(date):
    ...
    ret = "/usr/bin/spark2-submit --master yarn --queue root.ewhr_technical --deploy-mode client " + \
          ...
          "--class com.tmobile.sit.ignite.inflight.Application {}ignite-1.0-all.jar daily".format(WORK_FOLDER)
...
get_outputs_cmd = "cd {} && hdfs dfs -get {}* && rm cptm_ta_t_wlif_vchr_rds*".format(TMP_FOLDER, OUTPUT)

deliver_qv_cmd = "cd {} && scp -r *.csv hadoop@10.105.178.122:/Inflight/".format(TMP_FOLDER)

deliver_qs_cmd = "cd {} && scp -r *.csv cdrs@10.105.180.206:/Inflight/".format(TMP_FOLDER)

deliver_report_cmd = "mailx -a {} -s \"inflight_report_daily\" {} </dev/null".format(TMP_FOLDER+"reports/*", mail_to)
...
hdfs_put_yesterday = BashOperator(task_id='hdfs_put_yesterday',
                                  bash_command='hdfs dfs -put {} {}'.format(ARCHIVE_YESTERDAY, INPUT ),
                                  queue='EVL_Queue',
                                  dag=dag)

input_processing_yesterday = BashOperator(task_id='input_processing_yesterday',
                                          bash_command=spark_submit_template_yesterday + " && hdfs dfs -rm {}/*".format(INPUT),
                                          queue='EVL_Queue',
                                          dag=dag)


hdfs_put_today = BashOperator(task_id='hdfs_put_today',
                              bash_command='hdfs dfs -put {} {}'.format(ARCHIVE_TODAY, INPUT ),
                              queue='EVL_Queue',
                              dag=dag)



input_processing_today = BashOperator(task_id='input_processing_today',
                                      bash_command=spark_submit_template_today + " && hdfs dfs -rm {}/*".format(INPUT),
                                      queue='EVL_Queue',
                                      dag=dag)
...
deliver_qv = BashOperator(task_id='deliver_qv',
                          ...
                          queue='EVL_Queue',
                          dag=dag)
deliver_qs = BashOperator(task_id='deliver_qs',
                          bash_command=deliver_qs_cmd,
                          queue='EVL_Queue',
                          dag=dag)
...
kinit >> hdfs_put_yesterday >> input_processing_yesterday >> hdfs_put_today >> input_processing_today >> get_outputs >> deliver_qv >> deliver_qs >>deliver_reports >>clean