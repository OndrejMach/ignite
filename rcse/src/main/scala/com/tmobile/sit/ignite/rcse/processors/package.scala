...
import org.apache.spark.sql.functions.{asc, first, upper}
...
package object processors {
  ...
  implicit class Lookups(df: DataFrame)(implicit sparkSession: SparkSession) {
    ...
    def tacLookup(tacTerminal: DataFrame): DataFrame = {
      df
        .join(tacTerminal.select("tac_code", "terminal_id").distinct().cache(), Seq("tac_code"), "left_outer")
    }

    def terminalSWLookup(terminalSW: DataFrame): DataFrame = {
      df
        .join(terminalSW.select("rcse_terminal_sw_id", "rcse_terminal_sw_desc").distinct().cache(),
    ...
    def clientLookup(client: DataFrame): DataFrame = {
      df
        .join(client.select("rcse_client_id", "rcse_client_vendor_sdesc", "rcse_client_version_sdesc").distinct().cache(),
